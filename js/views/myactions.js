...
export async function renderMyActions(el) {
  ...
  el.innerHTML =
    ...
    '</div>' +
    '<div style="display:grid;grid-template-columns:2.3fr 1fr;gap:16px;align-items:start;">' +
      '<div>' +
        '<div class="panel" style="margin-bottom:16px;">' +
...
function renderMyActionsList() {
  ...
  listEl.innerHTML = rows.map(r => {
    ...
    const actionCell = pendingReq
      ? '<span class="pcf-pill pcf-pill-amber" style="flex-shrink:0;" title="' + String(pendingReq.comment || '').replace(/"/g,'&quot;') + '">Awaiting approval</span>'
      : r.eff === 'complete'
        ? '<span class="pcf-pill pcf-pill-blue" style="flex-shrink:0;">Complete</span>'
    ...
    return `
      <div class="ma-row" onclick="viewProduct('${r.prod.id}','${r.task.id}')">
        ...
        <span class="pcf-pill ${prioCls}" style="flex-shrink:0;">${r.priority}</span>
        <div class="ma-avatar" title="${ownerName}">${maInitials(ownerName)}</div>
        <div style="font-size:11px;color:${['overdue','delayed'].includes(r.eff) ? 'var(--red)' : 'var(--text-mid)'};flex-shrink:0;width:100px;">${maDueLabel(r.task.deadline)}</div>
        <span class="pcf-pill ${pill.cls}" style="flex-shrink:0;">${pill.label}</span>
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--border-mid)" stroke-width="2" style="flex-shrink:0;"><path d="M9 18l6-6-6-6"/></svg>
      </div>`;
  }).join('');
}
